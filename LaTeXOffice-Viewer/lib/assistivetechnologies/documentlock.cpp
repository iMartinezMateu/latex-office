...
const uint8_t iv[]  = { 0xf0, 0xe1, 0xd2, 0xc3, 0xb4, 0xa5, 0x96, 0x87, 0x78, 0x69, 0x5a, 0x4b, 0x3c, 0x2d, 0x5e, 0xaf }; //!<  Initialization Vector for 128-bit AES

inline int DocumentLock::getAlignedSize(int currSize, int alignment)
{
    Q_ASSERT(currSize >= 0);
...
QString DocumentLock::lockFile(QString rawText,  QString key)
{
...
QString DocumentLock::unlockFile(QString hexEncodedText,  QString key)
{
    QCryptographicHash hash(QCryptographicHash::Md5);
    hash.addData(key.toUtf8());
    QByteArray keyData = hash.result();
