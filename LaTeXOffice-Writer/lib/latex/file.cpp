...
void TeXFile::saveFile()
{
    ...
    if (!fileContentHistory.empty()) {
        if (fileContent != fileContentHistory.last()) { //Discarding no changes between commits.
...
void TeXFile::compile(QString compilerPath)
{
    ...
    if (QFile(compilerPath).exists() ) {
        ...
        QFile synctexFile(getFileName().append(".synctex"));
...
void TeXFile::precompile(QString compilerPath)
{
    if (QFile(compilerPath).exists()) {
        ...
        process->close();
        saveFile();
        qDebug() << QFile::copy(getFileName() + ".tmf",
                                getDirectory() + "/" + getFileName() + ".tmf");
    ...
    for (int line = 0; line < fileContent.length(); line++) {
        ...
        if (lineString.contains(CMD_LOGO)) {
            if (lineString.contains("/")) {
                ...
                replaceTeXCommand(command.callLogo(command.callIncludeGraphics(
                                                       getDirectory() + "/" + QFileInfo(
...
void TeXFile::deleteEmptyParagraphs()
{
    for (int line = 0; line < fileContent.length(); line++) {
        ...
        if (line+1< fileContent.length())
            if (fileContent.at(line).isEmpty() && fileContent.at(line+1).compare(QString(CMD_LINE_BREAK))==0 ) {
             deleteTeXCommand(line);
             deleteTeXCommand(line+1);
             qDebug() << "Deleted empty line";
           }
        if (fileContent.at(line).contains(QString(CMD_LINE_BREAK).append("}"))) {
            QString newLine  = fileContent.at(line);
            fileContent.replace(line, newLine.replace("\n", ""));
            qDebug() << "Deleted suspicious bracket";
        }
    }
    saveFile();
